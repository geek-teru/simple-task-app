...
func TestCreateUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    testdata.UserReqTestData[1],
			wanterr: nil,
		},
		{
			// 異常系: 一意制約違反
	...
	for _, tt := range tests {
		...
		userRepo.EXPECT().CreateUser(context.Background(), tt.args).Return(tt.want, tt.wanterr)

		got, goterr := userRepo.CreateUser(context.Background(), tt.args)
		// 結果の比較
